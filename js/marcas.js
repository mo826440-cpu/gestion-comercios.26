/* ============================================
   MARCAS - JAVASCRIPT
   ============================================
   Lógica de la pantalla de Marcas
============================================ */
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Pantalla de Marcas cargada');
    ...
    if (filterNombre) {
        ...
        filterNombre.addEventListener('keydown', function(e) {
            if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
                ...
                const resultados = document.querySelectorAll('#tbodyMarcas tr');
                ...
                e.preventDefault();
                const highlighted = document.querySelector('#tbodyMarcas tr.highlighted');
                if (highlighted) {
...
/**
 * Carga las marcas desde IndexedDB
 */
...
function actualizarIndicadores() {
    const totalEl = document.getElementById('totalMarcas');
    if (totalEl) {
        totalEl.textContent = marcasData.length;
    }
}
...
function aplicarFiltros() {
    marcasFiltradas = marcasData.filter(marca => {
        // Filtro por nombre
        if (filtroNombre) {
            const nombreMatch = marca.nombre?.toLowerCase().includes(filtroNombre);
...
function renderizarTabla() {
    ...
    // Calcular paginación
    const totalRegistros = marcasFiltradas.length;
    const totalPaginas = Math.ceil(totalRegistros / registrosPorPagina);
    const inicio = (paginaActual - 1) * registrosPorPagina;
    const fin = inicio + registrosPorPagina;
    const registrosPagina = marcasFiltradas.slice(inicio, fin);
        ...
        tbody.innerHTML = `
            <tr>
                <td colspan="6" style="text-align: center; padding: var(--espaciado-xl); color: var(--color-texto-secundario);">
                    ${filtroNombre ? 'No se encontraron marcas con ese nombre' : 'No hay marcas cargadas'}
                </td>
            </tr>
        `;
    } else {
        registrosPagina.forEach(marca => {
            const tr = document.createElement('tr');
            tr.dataset.id = marca.id;
            
            const fecha = marca.updated_at || marca.created_at || new Date().toISOString();
            const fechaFormateada = formatearFecha(fecha);
            const estado = marca.activo !== false ? 'Activa' : 'Inactiva';
            const estadoClass = marca.activo !== false ? 'activa' : 'inactiva';
            
            tr.innerHTML = `
                <td>${marca.id.substring(0, 8)}...</td>
                <td>${escapeHtml(marca.nombre || '')}</td>
                <td><span class="estado-badge ${estadoClass}">${estado}</span></td>
                <td>${fechaFormateada}</td>
                <td>${escapeHtml(marca.responsable_nombre || 'N/A')}</td>
                <td class="acciones-cell">
                    <button class="btn-accion editar" onclick="editarMarca('${marca.id}')" title="Editar">
                        ✏️ Editar
                    </button>
                </td>
            `;
...
function renderizarPaginacion(totalPaginas, totalRegistros) {
    ...
    let html = `
        <div class="paginacion-info">
            Mostrando ${inicio} - ${fin} de ${totalRegistros} marcas
        </div>
...
function cambiarPagina(nuevaPagina) {
    const totalPaginas = Math.ceil(marcasFiltradas.length / registrosPorPagina);
...
/**
 * Maneja el guardado de la marca
 */
async function manejarGuardar(e) {
    ...
    // Validaciones
    if (!nombre) {
        errorNombre.textContent = 'El nombre de marca es obligatorio';
    ...
    try {
        ...
        if (marcaEditando) {
            ...
            await db.marcas.update(marcaEditando.id, {
                ...cambios,
            ...
            const registroActualizado = await db.marcas.get(marcaEditando.id);
            if (typeof agregarAColaSincronizacion === 'function' && registroActualizado) {
                await agregarAColaSincronizacion('marcas', marcaEditando.id, 'update', registroActualizado);
            }
            
            mostrarNotificacion('Marca actualizada correctamente', 'exito');
        } else {
            // Crear nueva marca
            const nuevaMarca = {
                id: generarUUID(),
            ...
            await db.marcas.add(nuevaMarca);
            
            // Agregar a cola de sincronización
            if (typeof agregarAColaSincronizacion === 'function') {
                await agregarAColaSincronizacion('marcas', nuevaMarca.id, 'insert', nuevaMarca);
            }
            
            mostrarNotificacion('Marca creada correctamente', 'exito');
        }
        ...
        mostrarNotificacion('Error al guardar la marca: ' + error.message, 'error');
    }
}
...
async function editarMarca(id) {
    try {
        ...
        const marca = await db.marcas.get(id);
        if (!marca) {
        ...
        document.getElementById('especificaciones').value = marca.especificaciones || '';
        ...
        mostrarNotificacion('Error al cargar la marca', 'error');
    }
}
