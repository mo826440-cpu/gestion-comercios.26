/* ============================================
   CLIENTES - JAVASCRIPT
   ============================================
   Lógica de la pantalla de Clientes
============================================ */
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Pantalla de Clientes cargada');
    ...
    if (filterNombre) {
        ...
        filterNombre.addEventListener('keydown', function(e) {
            if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
                ...
                const resultados = document.querySelectorAll('#tbodyClientes tr');
                ...
                e.preventDefault();
                const highlighted = document.querySelector('#tbodyClientes tr.highlighted');
                if (highlighted) {
...
async function cargarClientes() {
    try {
        ...
        if (!db.clientes) {
            console.error('❌ La tabla clientes no existe en IndexedDB. Versión de BD:', db.verno);
...
function actualizarIndicadores() {
    ...
    if (totalEl) {
        totalEl.textContent = clientesData.length;
    }
    
    // Calcular deuda total (saldo pendiente a favor del comercio)
    const totalDeuda = clientesData.reduce((suma, cliente) => {
...
function aplicarFiltros() {
    clientesFiltrados = clientesData.filter(cliente => {
        // Filtro por nombre
        if (filtroNombre) {
            const nombreMatch = cliente.nombre?.toLowerCase().includes(filtroNombre);
...
function renderizarTabla() {
    ...
    const registrosPagina = clientesFiltrados.slice(inicio, fin);
        ...
        tbody.innerHTML = `
            <tr>
                <td colspan="9" style="text-align: center; padding: var(--espaciado-xl); color: var(--color-texto-secundario);">
                    ${filtroNombre ? 'No se encontraron clientes con ese nombre' : 'No hay clientes cargados'}
                </td>
            </tr>
        `;
    } else {
        registrosPagina.forEach(cliente => {
            const tr = document.createElement('tr');
            tr.dataset.id = cliente.id;
            
            const fecha = cliente.updated_at || cliente.created_at || new Date().toISOString();
            const fechaFormateada = formatearFecha(fecha);
            const estado = cliente.activo !== false ? 'Activa' : 'Inactiva';
            const estadoClass = cliente.activo !== false ? 'activa' : 'inactiva';
            const deuda = parseFloat(cliente.saldo_pendiente || 0);
            ...
            tr.innerHTML = `
                ...
                <td>${escapeHtml(cliente.documento || 'N/A')}</td>
                ...
                <td>${escapeHtml(cliente.responsable_nombre || 'N/A')}</td>
                <td class="acciones-cell">
                    <button class="btn-accion editar" onclick="editarCliente('${cliente.id}')" title="Editar">
                        ✏️ Editar
                    </button>
                </td>
            `;
...
async function manejarGuardar(e) {
    ...
    try {
        ...
        // Verificar que la tabla clientes existe
        if (!db.clientes) {
            console.error('❌ La tabla clientes no existe en IndexedDB');
            mostrarNotificacion('Error: La base de datos necesita actualizarse. Por favor, recargá la página (F5) o limpiá el caché del navegador.', 'error');
            return;
        }
        ...
        if (clienteEditando) {
            ...
            const registroActualizado = await db.clientes.get(clienteEditando.id);
            if (typeof agregarAColaSincronizacion === 'function' && registroActualizado) {
                await agregarAColaSincronizacion('clientes', clienteEditando.id, 'update', registroActualizado);
            }
            ...
            if (typeof agregarAColaSincronizacion === 'function') {
                await agregarAColaSincronizacion('clientes', nuevoCliente.id, 'insert', nuevoCliente);
            }
            
            mostrarNotificacion('Cliente creado correctamente', 'exito');
        }
        ...
        mostrarNotificacion('Error al guardar el cliente: ' + error.message, 'error');
    }
}
...
async function editarCliente(id) {
    try {
        ...
        document.getElementById('especificaciones').value = cliente.especificaciones || '';
